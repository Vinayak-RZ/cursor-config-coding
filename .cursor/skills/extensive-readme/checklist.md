# Extensive README — pre-ship checklist

## Routing and file

- [ ] Wrote `docs/EXTENSIVE.md` (or the path the user named), **not** as a replacement for a product/readable `README.md`
- [ ] Main `README.md` has a top banner link here
- [ ] Concepts and how-it-runs appear **before** the package map

## Coverage

- [ ] Domain concepts from *this* tree (not a named other product)
- [ ] End-to-end "how this repository runs" with a diagram
- [ ] **Every first-party package** has its own section
- [ ] Each package: what it is for, how it is used, how it works
- [ ] File map: path, **why it exists**, what it does
- [ ] No "and other utilities" instead of a section
- [ ] Generated/vendor trees mentioned once, not file-listed

## Accuracy

- [ ] Every path exists in the tree
- [ ] Package list matches workspaces / folders
- [ ] No invented features or URLs
- [ ] No private disk paths or customer names baked in as if they were this repo

## Teaching and future

- [ ] Non-obvious ideas explained simply; hard ones link a verified blog or wiki
- [ ] Future advancements: at least 3, grounded in this repo
- [ ] No "Despite its… faces challenges" closer; no "Future outlook" essay

## Anti-slop (required)

Run [../readme/anti-slop.md](../readme/anti-slop.md) **Pass**. Rewrite until clean.

- [ ] No "not just X, it's Y" punch
- [ ] No AI vocabulary stack, -ing tails, "serves as", em dashes, Title Case headings, chatbot residue
